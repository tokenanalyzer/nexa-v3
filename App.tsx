...
export default function App() {
  ...
  return (
    <SafeAreaView style={{flex:1, backgroundColor:T.bg}}>
      ...
      <View style={{flexDirection:'row', backgroundColor:T.surface, borderTopWidth:1, borderTopColor:T.card, paddingBottom:8}}>
        {TABS.map(t => (
          <TouchableOpacity key={t.id} onPress={() => setTab(t.id)}
            style={{flex:1, alignItems:'center', paddingVertical:10, gap:3}}>
            <Text style={{fontSize:20, opacity: tab===t.id ? 1 : 0.3}}>{t.icon}</Text>
            <Text style={{fontSize:8, fontWeight:'900', letterSpacing:1, color: tab===t.id ? T.accent : T.muted}}>{t.label}</Text>
          </TouchableOpacity>
        ))}
      </View>
    </SafeAreaView>
